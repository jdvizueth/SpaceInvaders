...
This module is like animation.py, except that it looks at
it can animate the ellipse in a circle, horizontally or
vertically.  We use key presses to switch between states.
...
class State(GameApp):
    ...
    Invariant: state is one of STATE_CIRCLE,
    STATE_HORIZONTAL, STATE_VERTICAL

    Attribute ellipse: the ellipse to animate
    Invariant: ellipse is a GEllipse whose center (in
    polar coordinates) is (RADIUS,self.angle) if
    STATE_CIRCLE, (factor+width/2,height/2) if
    STATE_HORIZONTAL, and (width/2,height/2+factor)
    if STATE_VERTICAL
    """
    ...
    def start(self):
        ...
        self.lastkeys = 0
        self.factor   = 0
        self.ellipse  = GEllipse(x=0,y=0,
                                 width=ELLIPSE_RADIUS,
                                 height=ELLIPSE_RADIUS,
                                 fillcolor=ELLIPSE_COLOR)

    def update(self,dt):
        """
        Determines the current state and animates the
        ellipse.
