...
class Invaders(GameApp):
    ...
    def draw(self):
        ...
        try:
            ...
            GLabel().draw(self.view)
        if self._state == STATE_ACTIVE or self._state == STATE_PAUSED:
            self._wave.draw(self.view)

    # HELPER METHODS FOR THE STATES GO HERE
    def _setupNewWave(self):
        """
        helps set up a new wave
        """
        self._text = None
        self._wave = Wave()
        self._state = STATE_ACTIVE

    def _setupActiveWave(self, dt):
        """
        runs the stuff that happens when state is active

        Parameter dt: The time in seconds since last update
        Precondition: dt is a number (int or float)
        """
        if self._wave.aliensLost():
            if self._wave.getAlienSpeed() > 0:
                self._wave = Wave(self._wave.getAlienSpeed() - 0.1,
                self._wave.getLives())
                print(str(self._wave.getAlienSpeed()))
            else:
                self._state = STATE_COMPLETE
        if self._wave.aliensWon():
            self._state = STATE_COMPLETE
        self._text = None
        self._wave.update(self.input, dt)
        if self._wave.getEnd():
    ...
    def _endscene(self):
        """
        decides which scenenario to use when the state is complete
        """
        if self._wave.aliensWon() or self._wave.getLives() <= 0:
            self._text = GLabel(text = 'Game Over', font_size = 24, \
            x = (GAME_WIDTH/2), y = (GAME_HEIGHT/2))
        elif self._wave.aliensLost():
            self._text = GLabel(text = 'You Won!!!', font_size = 24, \
            x = (GAME_WIDTH/2), y = (GAME_HEIGHT/2))
